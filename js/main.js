...
function createMovieDiv(filmObject) {
    ...
    <button type="button" data-id="${filmObject.id}" class="movie__btn"> Add to watchlist </button>
    </div>
    `
    return divEl;
};
...
const buttonsEl = document.getElementsByClassName('btns')[0];
if (buttonsEl) {
...
function createCartItemDiv(el) {
    ...
    <img src="img/${el.poster}" alt="${el.title}" class="movie__poste">;
    <div class="movie__info">
    <h2 class="movie__title">${el.title}</h2>
    <h3 class="movie__genre">${el.genres} </h3>
    <button type="button" data-index data-id="${el.id}" class="movie__btn delete__btn "> Remove from watchlist </button>
    </div>`;
    return divEl;
}
...
function showCartItems() {
    cartEl.innerHTML = ''
    let myCardArr = cartItemsFromLocalStorage();
    myCardArr.forEach((el) => {
        let movie = movies.filter((item) => {
            return item.id == el
        });
        let element = createCartItemDiv(movie[0]);
        cartEl.appendChild(element)
    });
}
if (cartEl) {
    cartEl.addEventListener('click', function (event) {
        if (event.target.className.includes('delete__btn'))
